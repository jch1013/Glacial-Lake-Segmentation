...
class unet_segmentation_model:
    ...
    def train_model(self, model_name, training_directory, epochs):
        image_dataset = get_image_dataset(training_directory, blur=True)
        labels_dataset = get_mask_dataset(training_directory, blur=True)
        ...
        if not os.path.exists(model_path):
            test = self.model.fit(X_train, y_train, batch_size=16, verbose=1, epochs=epochs, validation_data = (X_test, y_test), shuffle=False)
            self.model.save(model_path)
        else:
            print(f'The model {model_name} already exists. Please use a different name, or delete the model')
    ...
    def test_model(self, X_test, y_test):
        for i in range(len(X_test)):
            ...
            prediction = np.squeeze((self.model.predict(img) > 0.75).astype(np.uint8), axis=0)
...
test_model = unet_segmentation_model(256, 256, 3)
test_model.train_model('refactor_test_25_epoch', 'train_mini', 25)
